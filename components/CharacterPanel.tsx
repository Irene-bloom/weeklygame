  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="text-xs text-sub">周目旅行者</div>
          <div className="text-xl font-bold text-ink">{state.name}</div>
        </div>
        <div className="text-right">
          <div className="text-xs text-sub">周目等级</div>
          <div className="text-2xl font-bold text-accent">Lv.{playerLevel}</div>
        </div>
      </div>

      {/* 雷达图 */}
      <div className="flex justify-center rounded-2xl bg-card2 p-2 shadow-soft">
      ...
      {/* 季度 Boss 血条 */}
      <div className="rounded-2xl bg-card2 p-3 shadow-soft">
        <div className="mb-1 flex items-center justify-between text-xs">
          <span className="text-sub">本季 Boss · 拖延巨兽</span>
          <span className="font-semibold text-accent2">{bossPct}%</span>
        </div>
        ...
        <div className="mt-1 text-[11px] text-sub">每完成一次诚实复盘，对 Boss 造成伤害</div>
      </div>

      {/* 徽章墙 */}
      <div>
        <div className="mb-2 text-xs text-sub">徽章墙（{state.badges.length}/{BADGES.length}）</div>
            ...
            return (
              <div
                ...
                className={`flex aspect-square items-center justify-center rounded-xl text-xl transition ${
                  got ? "bg-gold/15 ring-1 ring-gold/40" : "bg-card2 opacity-40 grayscale"
                }`}
              >
                {b.emoji}
              </div>
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl bg-card2 px-2 py-2 shadow-soft">
