...
## 2026-06-06 · #005 修复 Vercel 构建失败 + 输入框对比度

**改了什么**
- **修 Vercel 构建失败**：把 Next.js 从 15.1.6 升到 **15.5.19**（Vercel 报 "Vulnerable version of Next.js detected" 拒绝构建）。顺手把 postcss 升到 `^8.5.10` 清掉审计告警。
- 本地跑 `npm run build` 生产构建通过（`✓ Compiled successfully`，等同 Vercel 流程），确认这次能部署成功。
- **修输入框文字看不清**：textarea 背景改纯白、文字加粗到 `text-[15px] font-medium text-ink`（深棕），占位符保持浅灰——打字内容对比度拉满。

**为什么**
- 用户在 Vercel 部署时构建失败，根因是当初 package.json 里 Next.js 版本写低了、含已知漏洞。
- 用户反馈输入框文字太黑/看不清，实为输入文字与底色对比不足，调白底 + 加粗深棕字解决。

**结论**
- 本地生产构建通过，推送后 Vercel 应能成功部署。输入框清晰可读。
